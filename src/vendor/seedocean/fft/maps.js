...
export function createCascadeMaps(cascade, { N, lambda, dt, foamDecay }) {
  ...
  const advect = Fn(() => {
    const id = instanceIndex;
    const coord = uvec2(id.mod(uint(N)), id.div(uint(N)));
    const uv = vec2(coord).add(0.5).div(float(N));
    ...
    const Nf = float(N);
    const backUV = vec2(uv.x.sub(dx.mul(dt).div(float(cascade.lengthScale))), uv.y.sub(dz.mul(dt).div(float(cascade.lengthScale))));
    const prev = texture(foamFront, backUV);

    // retention in [0,1): closer to 1 = foam persists longer.
    const retention = exp(dt.mul(float(1).sub(foamDecay).mul(-2.5).sub(0.12)));
  ...
  // Stable texture identities: advect reads A and writes B; commit copies B
  // back to A in a separate dispatch. Swapping JS variables after the TSL
  // graph was built did not swap its captured texture bindings.
  const commit = Fn(() => {
    const id = instanceIndex;
    const coord = uvec2(id.mod(uint(N)), id.div(uint(N)));
    const uv = vec2(coord).add(0.5).div(float(N));
    textureStore(foamA, coord, texture(foamB, uv)).toWriteOnly();
  })().compute(N * N);

  return { displacement, derivatives, assemble, advect, commit, foam: foamA };
}
